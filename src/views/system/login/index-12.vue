<script setup>
  ...
  const startAngle = -180 + totalAngle.value / 2 - props.gap / 2; // 初始角度
  let startTouchAngle = 0; // 手指按下时的角度
  let deltaAngle = 0; // 当前拖动的角度变化
  const maxOffsetAngle = -102.5 - startAngle;

  console.log("最大偏移量", maxOffsetAngle);
  // 计算扇形路径和图标位置
  const updatePaths = () => {
  ...
  const onTouchMove = (event) => {
    ...
    if (Math.abs(offsetAngle) > Math.abs(maxOffsetAngle) && offsetAngle < maxOffsetAngle) {
      deltaAngle = maxOffsetAngle + (offsetAngle - maxOffsetAngle) * 0.3;
    } else if (Math.abs(offsetAngle) > Math.abs(maxOffsetAngle) && offsetAngle > maxOffsetAngle) {
      deltaAngle = -maxOffsetAngle + (offsetAngle + maxOffsetAngle) * 0.3;
    } else {
      deltaAngle = offsetAngle;
    }

    console.log(offsetAngle, deltaAngle, currentTouchAngle);
