<script setup>
  ...
  // 计算扇形路径和图标位置
  const updatePaths = () => {
    angleOffset = startAngle + deltaAngle; // 基于拖动角度的偏移
  ...
  const onTouchMove = (event) => {
    ...
    // 限制最大旋转角度
    if (Math.abs(offsetAngle) > Math.abs(maxOffsetAngle)) {
      deltaAngle =
        maxOffsetAngle * Math.sign(-offsetAngle) +
        (offsetAngle - maxOffsetAngle * Math.sign(-offsetAngle)) * 0.3;
    } else {
