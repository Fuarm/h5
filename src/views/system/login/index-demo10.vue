<script setup>
  ...
  const targetStartAngle = -102.5;
  let startAngle = -180 + totalAngle.value / 2 - props.gap / 2; // 初始角度
  let startTouchAngle = 0; // 手指按下时的角度
  let deltaAngle = 0; // 当前拖动的角度变化

  // 计算扇形路径和图标位置
  const updatePaths = () => {
    let angleOffset = startAngle + deltaAngle; // 基于拖动角度的偏移
  ...
  const onTouchMove = (event) => {
    ...
    //   offsetAngle = -360 - startTouchAngle + currentTouchAngle;
    // } else {
    //   offsetAngle = currentTouchAngle - startTouchAngle;
    // }
    //
    // // 限制角度偏移范围
    // if (startAngle + offsetAngle < targetStartAngle) {
    //   deltaAngle = reboundTargetAngle + maxOffsetAngle + (offsetAngle - maxOffsetAngle) * 0.3;
    // } else {
    //   deltaAngle = reboundTargetAngle + offsetAngle;
    //   maxOffsetAngle = offsetAngle;
    // }

    deltaAngle = currentTouchAngle - startTouchAngle;
