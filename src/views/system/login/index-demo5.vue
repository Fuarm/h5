<script setup>
  ...
  // 最小和最大滑动范围限制
  const minAngle = -10; // 最小角度限制，防止拖动过远
  const maxAngle = 10; // 最大角度限制，防止拖动过远
  ...
  const onTouchMove = (event) => {
    ...
    // 超过拖动范围时应用回弹效果
    if (currentTouchAngle < minAngle) {
      // 计算超出部分
      const overshoot = minAngle - currentTouchAngle;
      deltaAngle.value = currentTouchAngle - startTouchAngle.value - overshoot * 0.3; // 应用柔和的回弹效果
    } else if (currentTouchAngle > maxAngle) {
      // 计算超出部分
      const overshoot = currentTouchAngle - maxAngle;
      deltaAngle.value = currentTouchAngle - startTouchAngle.value + overshoot * 0.3; // 应用柔和的回弹效果
    } else {
      deltaAngle.value = currentTouchAngle - startTouchAngle.value; // 计算正常的拖动角度差
    }
