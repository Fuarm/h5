<script setup>
  ...
  const onTouchMove = (event) => {
    ...
    // 计算当前角度与起始角度之间的差值
    let offsetAngle = currentTouchAngle - startTouchAngle.value;

    // 处理跨越边界的情况，确保角度变化连续
    if (Math.abs(offsetAngle) > 90) {
      // 如果角度跨越了边界（超过 180），则做循环修正
      if (offsetAngle > 0) {
        offsetAngle -= 360; // 处理从正到负的边界跨越
      } else {
        offsetAngle += 360; // 处理从负到正的边界跨越
      }
    }

    // 最大偏移量
    const maxOffsetAngle = 10;

    // 限制角度偏移范围
    if (Math.abs(offsetAngle) < maxOffsetAngle) {
      deltaAngle.value = offsetAngle;
    } else {
      deltaAngle.value =
        offsetAngle > 0 ? maxOffsetAngle + offsetAngle * 0.3 : -maxOffsetAngle + offsetAngle * 0.3;
    }
