<script setup>
  ...
  const onTouchMove = (event) => {
    ...
    // 限制角度偏移范围
    if (startAngle.value + offsetAngle < targetStartAngle) {
      deltaAngle.value = reboundTargetAngle + maxOffsetAngle + (offsetAngle - maxOffsetAngle) * 0.3;
    } else {
      deltaAngle.value = reboundTargetAngle + offsetAngle;
      maxOffsetAngle = offsetAngle;
    }

    console.log(startAngle.value + offsetAngle, maxOffsetAngle, targetStartAngle);
  ...
  const onTouchEnd = () => {
    ...
    reboundTargetAngle = targetStartAngle - startAngle.value; // 目标角度是0
