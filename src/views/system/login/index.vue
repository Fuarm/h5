<script setup>
  ...
  const props = defineProps({
    radius: { type: Number, default: 160 }, // 外圆半径
    innerRadius: { type: Number, default: 85 }, // 内圆半径
  ...
  let lastClockwiseOrAnticlockwise = null;
  // 手指移动事件，计算拖动角度
  const onTouchMove = (event) => {
    ...
    // 顺时针
    if (getClockwiseOrAnticlockwise(startTouchAngle, currentTouchAngle) === "clockwise") {
      // 判读起点 - 滑动重点
    } else {
      // 逆时针
    }
  ...
  const onTouchEnd = () => {
    ...
    // let endTouchAngle = getTouchAngle(event);

    // setClockwiseOrAnticlockwise(startTouchAngle, endTouchAngle);

    console.log(lastClockwiseOrAnticlockwise);
  ...
  // 计算旋转方向
  const getClockwiseOrAnticlockwise = (startAngle, endAngle) => {
    // 计算角度差
    ...
    if (angleDifference > 0) {
      console.log("顺时针旋转");
      return "clockwise";
      // lastClockwiseOrAnticlockwisekwise = "clockwise";
    } else if (angleDifference < 0) {
      console.log("逆时针旋转");
      // lastClockwiseOrAnticlockwise = "anticlockwise";
